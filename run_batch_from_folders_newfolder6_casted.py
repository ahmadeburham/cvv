...
from id_card_pipeline_casted import find_first_image, list_images, write_json
...
OUTPUTS_DIR = ROOT / "outputs_casted"
...
def run_batch(skip_face: bool = False) -> Path:
    ...
    summary = []
    for idx, id_path in enumerate(ids, start=1):
        print(f"[{idx}/{len(ids)}] {id_path.name}")
        out_json = OUTPUTS_DIR / f"{id_path.stem}.json"
        debug_dir = DEBUGS_DIR / id_path.stem
        cmd = [
            sys.executable,
            str(ROOT / "id_card_pipeline_casted.py"),
